...
// Esta função será executada apenas quando o documento HTML estiver completamente carregado.
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function adicionarAoCarrinho() {
        ...
        try {
            ...
            const produto = data.estoque.find(p => p.id.toLowerCase() === produtoId.toLowerCase());
            
            if (!produto) {
                const userResponse = confirm(`O produto com o ID "${produtoId}" não foi encontrado. Deseja adicionar este produto ao estoque agora?`);
                if (userResponse) {
                    showTab('#estoque');
                    DOM.abrirModalEstoqueBtn.click();
                    DOM.novoProdutoIdInput.value = produtoId;
                    DOM.novoProdutoNomeInput.focus();
                } else {
    ...
    if (DOM.tabButtons && DOM.tabContents) {
        ...
        console.error("Erro: Elementos 'tabButtons' ou 'tabContents' não encontrados.");
    }
    
    // Verificações adicionais antes de adicionar listeners
    if (DOM.adicionarProdutoBtn) DOM.adicionarProdutoBtn.addEventListener('click', adicionarAoCarrinho);
    if (DOM.finalizarVendaBtn) DOM.finalizarVendaBtn.addEventListener('click', finalizarVenda);
    if (DOM.listaCarrinho) {
    ...
    if (DOM.valorRecebidoInput) DOM.valorRecebidoInput.addEventListener('input', calcularTroco);

    if (DOM.relatorioDiarioBtn) DOM.relatorioDiarioBtn.addEventListener('click', () => gerarRelatorio('diario'));
    if (DOM.relatorioMensalBtn) DOM.relatorioMensalBtn.addEventListener('click', () => gerarRelatorio('mensal'));
    if (DOM.relatorioGeralBtn) DOM.relatorioGeralBtn.addEventListener('click', () => gerarRelatorio('geral'));
    if (DOM.relatorioDeliveryBtn) DOM.relatorioDeliveryBtn.addEventListener('click', () => gerarRelatorio('delivery'));
    if (DOM.enviarEmailBtn) DOM.enviarEmailBtn.addEventListener('click', enviarRelatorioPorEmail);

    if (DOM.fecharModalBtn) DOM.fecharModalBtn.addEventListener('click', fecharModalSenha);
    if (DOM.confirmarCancelamentoBtn) {
    ...
    if (DOM.abrirModalEstoqueBtn) DOM.abrirModalEstoqueBtn.addEventListener('click', () => {
        DOM.modalEstoque.style.display = 'flex';
    });
    if (DOM.fecharModalEstoqueBtn) DOM.fecharModalEstoqueBtn.addEventListener('click', () => {
        DOM.modalEstoque.style.display = 'none';
    });

    if (DOM.salvarNovoProdutoBtn) DOM.salvarNovoProdutoBtn.addEventListener('click', salvarNovoProduto);

    if (DOM.listaVendasDiarias) {
    ...
    // --- Carrega dados iniciais ao iniciar a página ---
    carregarEstoque();
    carregarAlertasEstoque();
    carregarHistoricoVendasDiarias();
    if (DOM.valorRecebidoInput) DOM.valorRecebidoInput.style.display = 'block';
});